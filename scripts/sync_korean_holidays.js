...
async function fetchHolidays(year, apiKey) {
  ...
  return items
    ...
    .map(item => ({ date: toDateKey(item.locdate), name: String(item.dateName || '법정공휴일').trim() }))
...
async function main() {
  ...
  const holidays = (await Promise.all(yearsToSync().map(year => fetchHolidays(year, apiKey)))).flat();
